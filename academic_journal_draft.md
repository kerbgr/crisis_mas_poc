...
**Scale normalisation requirement.** A critical methodological constraint of the 60/40 blending formula is that both components must share the same distributional scale for the weights to be mathematically meaningful. ER and GAT aggregated beliefs are proper probability distributions that sum to 1.0 across all alternatives; their per-alternative average is therefore $1/N$ (approximately 0.20 for $N=5$ and 0.083 for $N=12$). TOPSIS closeness coefficients $C_i = S_i^-/(S_i^+ + S_i^-)$, however, are geometric proximity scores that are individually bounded in $[0,1]$ but do not sum to 1. In the experimental corpus, raw TOPSIS scores sum to 1.8-3.2 across alternatives, making the effective MCDA contribution 55-79\% of the combined score despite the nominal 40\% weight. This distortion is proportional to the ratio of average TOPSIS score to average belief mass and therefore grows with action-space size. To restore the intended balance, TOPSIS raw scores are L1-normalised before combination: $C_k^{\text{norm}} = C_k / \sum_j C_j$. This preserves the TOPSIS ranking order while ensuring both components are proper distributions that sum to 1. The corrected formula $\text{Score}(A_k) = 0.6 \times m_\text{agg}(A_k) + 0.4 \times C_k^{\text{norm}}$ is used in all analyses reported in this paper, and a post-hoc recalculation over all 92 stored result files confirmed that the correction changes 11 of 30 Evia Wildfire recommendations (from `action_combined_assault` to `action_immediate_evacuation` or `action_phased_evacuation`) while leaving Flood and HAZMAT results unaffected (see Section VI.A, Finding 10).
...
**Scale normalisation finding.** Post-hoc analysis across all 92 stored result files revealed a systematic scale incompatibility in the 60/40 DQS blending formula (described in Section III.D). Because raw TOPSIS closeness coefficients are geometric proximity scores summing to 1.8-3.2 across alternatives — rather than proper probability distributions summing to 1.0 like ER/GAT beliefs — the MCDA component was contributing 55-79% of the combined DQS despite its nominal 40% weight. The distortion scales with action-space size: it is modest for $N=5$ but reaches 76% for $N=12$. L1 normalisation of TOPSIS scores before blending restores the intended balance and changed 11 of 30 Evia Wildfire recommendations (6 ER + 5 GAT), all from `action_combined_assault` to `action_immediate_evacuation` or `action_phased_evacuation` — alternatives that better reflect the collective agent belief distribution. Flood and HAZMAT scenarios were unaffected (1 and 0 changes respectively), confirming the effect is specific to large action spaces. This finding also has implications for future work: any supervised training of the GAT attention weights on the 1,133-record corpus requires L1-normalised DQS signals as labels; training on raw scores would encode the scale bias.
