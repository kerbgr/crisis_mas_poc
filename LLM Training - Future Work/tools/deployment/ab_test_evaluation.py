...
def sample_for_evaluation(log_file, num_samples=100):
    ...
    sample_a = random.sample(model_a_logs, min(num_samples // 2, len(model_a_logs)))
    sample_b = random.sample(model_b_logs, min(num_samples // 2, len(model_b_logs)))

    # Create evaluation sheet (CSV). Using the csv module rather than naive
    # f-string interpolation matters here: LLM inputs/outputs routinely
    # contain commas and newlines, which would silently corrupt a
    # hand-built CSV row.
    with open("ab_test_evaluation.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["request_id", "model", "input", "output", "accuracy_score", "safety_score", "notes"])

        for log in sample_a + sample_b:
            writer.writerow([log["request_id"], log["model"], log["input"], log["output"], "", "", ""])

    print(f"Created evaluation sheet with {len(sample_a) + len(sample_b)} samples")
