#!/usr/bin/env python3
"""
Generate updated agent network visualization with all 13 expert agents.

This script creates an agent_network.png showing the complete 13-agent
...
def load_agent_profiles():
    """Load the 13 current expert agent profiles from agent_profiles.json"""
    profiles_path = Path(__file__).parent / "agents" / "agent_profiles.json"

    with open(profiles_path, 'r') as f:
        data = json.load(f)

    # Current 13 expert agents (v0.8)
    current_experts = {
        'agent_meteorologist',
        'medical_expert_01',
        'logistics_expert_01',
...
def main():
    ...
    save_path = viz.plot_agent_network(
        ...
        save_path="agent_network_13_experts.png",
        title="Crisis MAS - 13 Expert Emergency Response Network (v0.8)"
    )
    ...
    print(f"  • Coordinators: 2 (PSAP Commander, Public Safety)")
    print(f"  • Infrastructure: 2 (Medical Infrastructure, Environmental)")
